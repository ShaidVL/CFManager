<template>
  <div>
    ...
    <div>
      ...
      <button @click="getLoan">Получить кредит банка</button>
    </div>
    <hr>
    <div>
      <input v-model="repayCredit" name="repayCredit" type="text" @input="checkForDigit">
      <button @click="repayLoan">Погасить кредит банка</button>
    </div>
    <hr>
    <div>
    ...
    <div v-if="showLiability">
      <div v-for="liability in this.repayLiabilities">
        ...
        <div><button @click="removeLiability(liability)">Убрать</button></div>
        <hr>
      </div>
    </div>
  </div>
